...
namespace Soccer.Controllers
{
    public class TeamsController : Controller
    {
        ...
        // GET: Teams
        public async Task<IActionResult> Index(string? coach)
        {
            var teamsQuery = _context.Teams.AsQueryable();

            if (!string.IsNullOrEmpty(coach)) {
                teamsQuery = teamsQuery.Where(t => t.Coach!.Contains(coach));
            }

            var viewModel = new UserListViewModel<Teams> {
                Item = await teamsQuery.ToListAsync(),
                Position = coach
            };
